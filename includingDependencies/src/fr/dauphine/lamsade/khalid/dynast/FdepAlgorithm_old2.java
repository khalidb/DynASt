...
public class FdepAlgorithm_old2 implements FunctionalDependencyAlgorithm,
    ...
    protected ObjectArrayList<HashMap<Integer,ArrayList<Boolean>>> evidenceSets;
...
//				System.out.println(fdResult.toString());
	...
	public void initializeEvidenceSets() {
		...
		evidenceSets = new ObjectArrayList<HashMap<Integer,ArrayList<Boolean>>>();
		
		for (int i=0; i<this.numberAttributes; i++) {
			System.out.println("Evidence sets for attribute: "+i);
			
			HashMap<Integer,ArrayList<Boolean>> tmp = new HashMap<Integer,ArrayList<Boolean>>();
			
			for (Integer key_j : this.getTuples().keySet()) {
			
			//for (int j=0; j< this.getTuples().size(); j++) {
				ArrayList<Boolean> evs_j =  new ArrayList<Boolean>();
	...
	public static void main(String[] args) {
		...
		FdepAlgorithm_old2 algo;
        algo = new FdepAlgorithm_old2();
...
*/		
	...
	public void insertBitsets(int n) {
			...
			for (int j =0; j<this.numberAttributes; j++) 
				vecs.add(this.evidenceSets.get(j).get(n));
	...
	public void deleteBitsets() {
		...
		for (int k=0; k<this.deleted_tuples.size(); k++) {
			...
			for (int j =0; j<this.numberAttributes; j++) 
				vecs.add(this.evidenceSets.get(j).get(this.deleted_tuples.get(k)));
