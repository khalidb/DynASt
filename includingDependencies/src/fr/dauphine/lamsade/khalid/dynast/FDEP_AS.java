...
class FDEP_AS {
     ...
     */
    public HashMap<OpenBitSet,Integer> bitesetMap = new HashMap<>();
    public HashMap<OpenBitSet,Integer> bitsets_cardinality_reduced;
    public HashMap<OpenBitSet,Integer> bitesetMap_insert = new HashMap<>();
    public HashMap<OpenBitSet,Integer> bitesetMap_delete = new HashMap<>();
    public HashMap<OpenBitSet,Integer> bitesetMap_after_delete = new HashMap<>();
    ...
    public FDEP_AS(int numAttributes, ValueComparator valueComparator) {
...
//        addAllDependenciesToResultReceiver();
	...
	public int getNumberOfEvidenceSets() {
		
		int size = 0;
		
		for (HashMap map: AVES) {
			size+= map.size();
		}
	 ...
	 * The following method update the bitsetmap following the insertion of new tuples
	 */
	public void deleteBitsets_New(List<Long> delete_duration) {
		...
		bitsets_cardinality_reduced = new HashMap<OpenBitSet,Integer>();
		...
		for (int k=0; k<this.deleted_tuples.size(); k++) {
			...
			for (int j =0; j<this.numberAttributes; j++) {	
	    	 ...
	    	 for (int i=0;i<vecs.get(0).size(); i++) {
	    		 ...
	    		 if (!bs.equals(emptybitset))
	    			 bitsets_cardinality_reduced.merge(bs, 1, Integer::sum);
    ...
    */
	...
	public void updateEvidenceGivenDeletion_New(int tuple_id, List<Integer> deleted_tuple) {
		 ...
		 int idx_tuple = tuple_ids.indexOf(tuple_id);
			for (int att =0; att<this.numberAttributes; att++) {
				HashMap<Integer,DiffVector> temp_hm = this.AVES.get(att);
				for (Integer key_v: this.AVES.get(att).keySet()) {
						//this.AVES.get(att).get(key_v).remove(idx_tuple); 
						temp_hm.get(key_v).remove(idx_tuple);
					}
			}
	...
	public void old_main() {
		...
		FDEP_AS algo;
        algo = new FDEP_AS(4, new ValueComparator(true));
	...
	public static void main(String[] args) {
        ...
        FDTree fdtree = new FDTree(4,0);
		OpenBitSet lhs1 = new OpenBitSet();
		lhs1.set(0);
		lhs1.set(3);
		OpenBitSet lhs2 = new OpenBitSet();
		lhs2.set(0);
		lhs2.set(1);
		lhs2.set(3);
				
		fdtree.addFunctionalDependency(lhs1, 1); 
		fdtree.addFunctionalDependency(lhs2, 1); 
		fdtree.printDependencies(4);
		
		
		List<OpenBitSet> bitset = fdtree.getFdAndGeneralizations(lhs2, 1);
		System.out.println("Generalizations of "+BitSetUtils.toString(lhs2,4));
		for (OpenBitSet bit: bitset) {
			System.out.println(BitSetUtils.toString(bit,4));
		}
